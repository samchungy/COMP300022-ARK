...
//                    Intent myIntent3 = new Intent(HomeActivity.this, ArActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mCurrentUser.getEmail()==null&&ARK_auth.fetchUserEmail(getApplicationContext())!="user does not exist"){
            mCurrentUser.logOn(ARK_auth.fetchUserEmail(getApplicationContext()),getApplicationContext());
            UserRequestsUtil.initialiseCurrentUser(getApplicationContext());
        }

        //showToast(ARK_auth.fetchSessionId(getApplicationContext()));
        if(ARK_auth.fetchSessionId(getApplicationContext()).equals("no user email")) {
